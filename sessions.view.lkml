view: sessions {
  derived_table: {
    sql: SELECT
        ...
        , SUM(CASE WHEN e.event_type IN ('Category','Brand') THEN 1 END) AS browse_events
        , SUM(CASE WHEN e.event_type = 'Product' THEN 1 END) AS product_events
        , SUM(CASE WHEN e.event_type = 'Cart' THEN 1 END) AS cart_events
        , SUM(CASE WHEN e.event_type = 'Purchase' THEN 1 end) AS purchase_events
        , MAX(e.user_id) AS session_user_id
        , MIN(e.id) AS landing_event_id
        , MAX(e.id) AS bounce_event_id
      FROM events as e
      GROUP BY session_id
       ;;
...
#     timeframes: [time, date, week, month, hour_of_day, day_of_week]
  ...
  measure: overall_conversion {
    ...
    drill_fields: [events.landing_event*]
  }
